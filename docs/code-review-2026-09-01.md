...
- **Observed flake.** During this review the full suite was run five times; four runs were fully green and one run reported 2 failures out of 346 that did not reproduce (the failing names were not captured because the run was tailed). The suite has no per-test timeout, so timing-sensitive cross-process tests are the likely source; treat this as a reason for the configuration below rather than as a specific bug.
